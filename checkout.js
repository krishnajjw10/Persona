...
checkoutForm.addEventListener('submit', async event => {
  ...
  try {
    ...
    if (result.mode === 'local-test') {
      ...
      checkoutStatus.textContent = `Order ${order.orderId} saved.`;
    }

    checkoutForm.reset();
    renderSummary();
  } catch (error) {
    checkoutStatus.textContent = 'Could not save order. Check your sheet endpoint and try again.';
